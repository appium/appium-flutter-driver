...
| appium:dartVmServicePort | **iOS only.** Pin the Dart VM service to a known port at app launch. The driver injects both `--vm-service-port=<port>` (canonical, Flutter ≥3.10) and `--observatory-port=<port>` (legacy, Flutter <3.10) into `processArguments.args`, plus `--disable-service-auth-codes`, so the Flutter engine deterministically binds to this port regardless of which Flutter version the app bundles. Also used as the fallback URL when the syslog scan exhausts without finding the Dart VM service URL (e.g. when iOS unified-logging privacy filters drop the "Dart VM service is listening on …" line). Existing `--vm-service-port`/`--observatory-port` entries in `processArguments.args` are stripped and replaced. Does not apply when `observatoryWsUri` is also set (the latter takes precedence for connection). | 8283
